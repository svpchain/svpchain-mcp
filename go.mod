...
// The MCP server is a client of the svpchain protocol module: it reuses the
// generated x/*/types proto messages, the app/config address prefixes, and the
// app/module + app/basic_manager interface registry. The protocol module is
// consumed from a sibling checkout of the monorepo (no published tag needed);
// point this at wherever protocol/ lives if the layout differs.
replace github.com/dydxprotocol/v4-chain/protocol => ../svpchain-main/protocol

// The replace blocks below are copied VERBATIM from protocol/go.mod. Go does
// not apply a dependency module's own replace directives, so any module that
// depends on the protocol module (a Cosmos chain, with its forks of cosmos-sdk,
// cometbft, cosmos/evm, iavl, etc.) MUST redeclare them here or the build
// resolves the upstream modules instead of the forks and fails to compile.
...
replace (
	...
	// Pin collections to v0.4.0 for compatibility with cosmossdk.io/core v0.11.0
	...
	// deltaping fork of cosmos-sdk store — root-only app hash + LatestVersion perf fix
...
replace (
	...
	// Use Cosmos geth fork (required by cosmos/evm)
