...
#                                  tool family. Default http://127.0.0.1:8545
#                                  (the node's local JSON-RPC port). Set ""
...
faucet_address="${SVPCHAIN_FAUCET_ADDRESS:-}"
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --faucet-address)         faucet_address="$2";    shift 2 ;;
...
# apply, see cmd/mcp-server/config.go::LimitsConfig). The EVM block is
# emitted when --evm-rpc is non-empty (default on); the [evm.faucet]
# sub-section only when --faucet-address is set.
render_mcp_toml() {
  cat <<EOF
...
EOF
  # evm_rpc_url is a top-level key, so it must precede any [table] header.
